...
% Paremeter (weighting of important topics specified by the user)
weight = 5;
...
%     Allow user to choose most important topics manually
    prompt2 = 'Would you like to choose important topics manually?(y/n)\n';
    response = input(prompt2,'s');
    more_input = strcmp(response,'y') || strcmp(response,'Y');
    
    topic_choices = zeros(num_topics,1);
    count = 0;
    while(more_input && count < num_topics)
        display('Top ten words in each topic:\n');
        display(celldict(top_words(U,10)));
        prompt3 = 'Enter the number of a topic related to the search document (e.g. 3):\n';
        prompt4 = 'Response recorded. Would you like to enter another topic? (y/n):\n';
        response = input(prompt3);
        if (isnumeric(response))
            count = count + 1;
            topic_choices(count) = response;
            response = input(prompt4,'s');
            if strcmp('n',response) || strcmp('N',response)
                more_input = false;
            end
        end
    end
    ...
    if count > 0
        topic_choices(topic_choices==0) = [];
        topic__choices = unique(topic_choices);
    end
    
    clear more_input prompt2 prompt3 prompt4 count response;
...
%     Ask user if they'd like to search for more documents
